-- Manuális eladás: üres p_payments → unpaid (utalás / díjbekérő).
-- + record_sale_payment: későbbi kiegyenlítés unpaid/partial eladáson.
-- create_sale body: 20260514 alapján, fizetés-validáció patch.
...
-- ---------------------------------------------------------------------------
-- record_sale_payment — unpaid/partial eladás későbbi kiegyenlítése
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_sale.payment_status in ('paid', 'refunded') then
    return jsonb_build_object('ok', false, 'message', 'Az eladás már teljesen kiegyenlítve.');
  end if;
  ...
  v_paid_sum := v_paid_sum + v_amt;

  if v_paid_sum >= v_due - 1 then
    v_new_status := 'paid';
