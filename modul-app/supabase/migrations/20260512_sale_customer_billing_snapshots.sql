-- Sale customer/billing snapshots (Shopify/ERP-style freeze at fulfill time).
-- Backfill from customers for existing sales; create_sale writes snapshots going forward.
...
comment on column public.sales_orders.customer_email_snapshot is
  'Email at sale time (immutable for the document).';
comment on column public.sales_orders.billing_tax_number_snapshot is
  'Billing tax number at sale time.';

-- Backfill from live customer row when snapshot empty
update public.sales_orders s
set
  customer_email_snapshot = coalesce(s.customer_email_snapshot, c.email),
  customer_mobile_snapshot = coalesce(s.customer_mobile_snapshot, c.mobile),
  billing_name_snapshot = coalesce(s.billing_name_snapshot, c.billing_name),
  billing_country_snapshot = coalesce(
    s.billing_country_snapshot,
    nullif(trim(c.billing_country), ''),
    'Magyarország'
  ),
  billing_city_snapshot = coalesce(s.billing_city_snapshot, c.billing_city),
  billing_postal_code_snapshot = coalesce(s.billing_postal_code_snapshot, c.billing_postal_code),
  billing_street_snapshot = coalesce(s.billing_street_snapshot, c.billing_street),
  billing_house_number_snapshot = coalesce(s.billing_house_number_snapshot, c.billing_house_number),
  billing_tax_number_snapshot = coalesce(s.billing_tax_number_snapshot, c.billing_tax_number),
