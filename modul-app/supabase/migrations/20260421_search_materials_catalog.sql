-- Kereső P0: trigram indexek + egy roundtrip RPC

create extension if not exists pg_trgm;

-- ---------------------------------------------------------------------------
-- GIN trigram indexes (alive rows)
-- ---------------------------------------------------------------------------
create index if not exists sheet_materials_name_trgm_idx
  on public.sheet_materials using gin (name gin_trgm_ops)
  where deleted_at is null;

create index if not exists sheet_materials_machine_code_trgm_idx
  on public.sheet_materials using gin (machine_code gin_trgm_ops)
  where deleted_at is null and machine_code is not null;

create index if not exists linear_materials_name_trgm_idx
  on public.linear_materials using gin (name gin_trgm_ops)
  where deleted_at is null;

create index if not exists accessories_name_trgm_idx
  on public.accessories using gin (name gin_trgm_ops)
  where deleted_at is null;

create index if not exists accessories_sku_trgm_idx
  on public.accessories using gin (sku gin_trgm_ops)
  where deleted_at is null;

create index if not exists accessories_barcode_trgm_idx
  on public.accessories using gin (barcode gin_trgm_ops)
...
-- ---------------------------------------------------------------------------
-- search_materials_catalog — egy PostgREST roundtrip
-- ---------------------------------------------------------------------------
