      ...
      >
        <DialogHeader className="shrink-0 border-b border-border px-4 py-3 pr-10">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            <StatusBadge tone={isCash ? 'success' : 'neutral'}>
              {paymentMethodName || 'Fizetés'}
            </StatusBadge>
            <StatusBadge tone="neutral">{warehouseName}</StatusBadge>
            <StatusBadge tone={customerName ? 'info' : 'neutral'}>
              {customerName ?? 'Vendég'}
            </StatusBadge>
            {overstockCount > 0 ? (
        ...
        <div className="shrink-0 space-y-1.5 border-t border-border bg-subtle/50 px-4 py-3">
          ...
          {totals.globalDiscountAmount > 0 ? (
            <SummaryRow
              label={`Kedvezmény ${totals.globalDiscountPercent}%`}
