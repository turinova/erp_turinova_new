...
import {
  FooterGroup,
  HideOnScrollHeader,
  STOREFRONT_FOOTER_ID
} from '@/components/storefront/storefront-scroll'
...
export function StorefrontHeader({ seller }: { seller: StorefrontSeller }) {
  return (
    <HideOnScrollHeader>
      <div className="mx-auto flex h-[52px] max-w-[1200px] items-center justify-between gap-3 px-4 lg:px-8">
        <Link
...
export function StorefrontFooter({
  ...
  const host = [
    settings.hostingProviderName,
    settings.hostingProviderAddress,
    settings.hostingProviderEmail
  ].filter(Boolean)
  ...
  const hasCustomerInfo =
    legalLinks.length > 0 || Boolean(settings.complaintInfo) || host.length > 0
      ...
      <div className="mx-auto max-w-[1200px] px-4 pt-8 lg:px-8 lg:pt-10">
        {hasContact ? (
          <section aria-labelledby="footer-contact" className="pb-6 lg:pb-8">
            ...
            <p className="mt-0.5">Méret, szerelés, szállítás — kérdezz bátran.</p>
