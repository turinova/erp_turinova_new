...
export default async function UgyfelekListPage({
  searchParams
}: {
  ...
  if (user?.tenantId && !user.isDevSession) {
    ...
    loadError =
      'Dev bypass módban nincs tenant adatbázis. Kapcsold be a Supabase env-et, és futtasd a customers migrációt.'
  }

  if (loadError) {
    return (
      <div className="space-y-3">
        <h1 className="text-h1 text-ink">Ügyfelek</h1>
        <p
        ...
        <p className="max-w-xl text-body text-ink-secondary">
          ...
          <code className="text-hint">
            supabase/migrations/20260312_customers.sql
          </code>{' '}
