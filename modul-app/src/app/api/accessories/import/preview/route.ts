...
import { previewAccessoriesImport } from '@/lib/accessories/import-plan'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await previewAccessoriesImport(
      ctx.supabase,
      ctx.user.tenantId!,
      buffer
    )
    ...
    console.error('accessories import preview', err)
    return NextResponse.json(
      {
        error:
