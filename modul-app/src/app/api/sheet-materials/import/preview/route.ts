...
import { previewSheetMaterialsImport } from '@/lib/sheet-materials/import-plan'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await previewSheetMaterialsImport(
      ctx.supabase,
      ctx.user.tenantId!,
      buffer
    )
    ...
    console.error('sheet-materials import preview', err)
    return NextResponse.json(
      {
        error:
