...
export default function FeeTypeFormClient({ initialFeeType, isEdit }: FeeTypeFormClientProps) {
  ...
  const handleSave = async () => {
    ...
    if (!feeType.net_price || feeType.net_price <= 0) {
      newErrors.net_price = 'A nettó ár mező kötelező és nagyobb kell legyen mint 0'
    }
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 3 }}>
        <Grid container spacing={3}>
          ...
          {feeType.vat_id && feeType.currency_id && feeType.net_price > 0 && (
            <>
              <Grid item xs={12}>
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  ...
                  value={formatCurrency(
                    (feeType.net_price * (vatRates.find(v => v.id === feeType.vat_id)?.kulcs || 0)) / 100,
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  ...
                  label="Bruttó ár"
                  value={formatCurrency(
                    feeType.net_price + ((feeType.net_price * (vatRates.find(v => v.id === feeType.vat_id)?.kulcs || 0)) / 100),
                    currencies.find(c => c.id === feeType.currency_id)?.name
                  )}
                  InputProps={{ readOnly: true }}
                  variant="filled"
