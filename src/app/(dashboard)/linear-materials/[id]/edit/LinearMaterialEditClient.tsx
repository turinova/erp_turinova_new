...
export default function LinearMaterialEditClient({ initialLinearMaterial, brands, vatRates, currencies, priceHistory }: LinearMaterialEditClientProps) {
  ...
  // Calculate prices
  const netPricePerM = formData.price_per_m || 0
        ...
        <Grid item xs={12}>
          <Card>
            ...
            <CardContent>
              <Grid container spacing={3}>
                <Grid item xs={12} md={4}>
                  <TextField fullWidth required label="Ár/m (Ft)" type="number" value={formData.price_per_m} onChange={(e) => handleInputChange('price_per_m', parseFloat(e.target.value.replace(',', '.')) || 0)} inputProps={{ step: 0.01 }} />
                </Grid>
                <Grid item xs={12} md={4}>
