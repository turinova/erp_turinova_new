...
export async function middleware(req: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    ...
    return new NextResponse(
      JSON.stringify({ 
        ...
        message: 'Supabase environment variables are not set. Please check your .env.local file and restart the dev server.'
      }),
      { 
        status: 500,
  ...
  if (session && session.user) {
    // Allow /home for all authenticated users as a fallback (prevents redirect loops for users with no permissions)
    if (req.nextUrl.pathname === '/home') {
      if (isDev) console.log('Middleware - /home is accessible to all authenticated users')
      return response
    }
