...
export default function OrderInvoiceModal({
  ...
  const [invoiceEmail, setInvoiceEmail] = useState(() => String(order.customer_email ?? '').trim())
  ...
  useEffect(() => {
    if (!open) return
    setInvoiceEmail(String(order.customer_email ?? '').trim())
    setSendEmail(!!order.customer_email)
  }, [open, order.id, order.customer_email])
  ...
  useEffect(() => {
    if (open) {
      ...
      fetch(`/api/orders/${order.id}/invoices`)
        .then(res => res.json())
        .then(data => {
  ...
  const createTemplateProforma = useCallback(async (): Promise<string | null> => {
    ...
    try {
      const response = await fetch(`/api/orders/${order.id}/create-template-proforma`, {
        method: 'POST',
        headers: {
  ...
  const queryInvoicePdf = useCallback(async (invoiceNumber: string): Promise<string | null> => {
    try {
      const response = await fetch(`/api/orders/${order.id}/query-invoice-pdf`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const deleteTemplateProforma = useCallback(async (invoiceNumber: string | null) => {
    ...
    try {
      const response = await fetch(`/api/orders/${order.id}/delete-preview-invoice`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleCreateInvoice = async () => {
    ...
    if (sendEmail && !invoiceEmail.trim()) {
      setError('E-mail küldéshez adja meg a címzett e-mail címét.')
      return
    }
    ...
    try {
      const response = await fetch(`/api/orders/${order.id}/create-invoice`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        ...
        <Grid container spacing={3} sx={{ mt: 0 }}>
          ...
          <Grid item xs={12} md={4}>
            <Stack spacing={3}>
              ...
              <TextField
                fullWidth
                label="E-mail (számla értesítő)"
                type="email"
                autoComplete="email"
                value={invoiceEmail}
                onChange={(e) => setInvoiceEmail(e.target.value)}
                size="small"
                helperText="A Számlázz.hu erre küldi a számlát. Tesztfióknál a Tudástár szerint a fiók e-mailjére mehet az értesítő."
              />
              <FormControl fullWidth size="small">
