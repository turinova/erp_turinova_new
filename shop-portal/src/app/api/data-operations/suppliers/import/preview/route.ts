...
import { SUPPLIER_XLSX_COLUMNS, normalizeSupplierImportRows, detectDuplicateSupplierRows } from '@/lib/data-operations/suppliers-xlsx'

function validateHeaderColumns(columns: string[]): string[] {
  const missing = SUPPLIER_XLSX_COLUMNS.filter((col) => !columns.includes(col))
...
export async function POST(request: Request) {
  try {
    ...
    const normalized = detectDuplicateSupplierRows(normalizeSupplierImportRows(rawRows))
