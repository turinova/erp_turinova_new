...
import { PRODUCT_XLSX_COLUMNS, detectDuplicateProductRows, normalizeProductImportRows } from '@/lib/data-operations/products-xlsx'

function validateHeaderColumns(columns: string[]): string[] {
  const missing = PRODUCT_XLSX_COLUMNS.filter((col) => !columns.includes(col))
...
export async function POST(request: Request) {
  try {
    ...
    const normalized = detectDuplicateProductRows(normalizeProductImportRows(rawRows))
