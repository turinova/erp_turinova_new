...
import {
  PRODUCT_SUPPLIER_XLSX_COLUMNS,
  detectDuplicateProductSupplierRows,
  normalizeProductSupplierImportRows
} from '@/lib/data-operations/product-suppliers-xlsx'

function validateHeaderColumns(columns: string[]): string[] {
  const missing = PRODUCT_SUPPLIER_XLSX_COLUMNS.filter((col) => !columns.includes(col))
...
export async function POST(request: Request) {
  try {
    ...
    const normalized = detectDuplicateProductSupplierRows(normalizeProductSupplierImportRows(rawRows))
    ...
    console.error('Product supplier import preview error:', error)
