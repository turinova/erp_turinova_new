...
/**
 * GET /api/customers/companies/[id]/bank-accounts
...
export async function GET(
  ...
  try {
    ...
    // Fetch bank accounts (check both company_id and customer_entity_id for backward compatibility)
    ...
    console.error('Error in company bank accounts GET API:', error)
    return NextResponse.json(
...
/**
 * POST /api/customers/companies/[id]/bank-accounts
 * Add a new bank account to a company
 */
export async function POST(
  ...
  try {
    ...
    console.error('Error in company bank accounts POST API:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
