...
/**
 * GET /api/customers/persons/[id]/addresses/[addressId]
 * Fetch a single address for a person
 */
export async function GET(
  ...
  try {
    ...
    // Fetch address (check both person_id and customer_entity_id for backward compatibility)
    ...
    console.error('Error in person address GET API:', error)
    return NextResponse.json(
...
/**
 * PUT /api/customers/persons/[id]/addresses/[addressId]
...
export async function PUT(
  ...
  try {
    ...
    // Update person's default address references if needed
    if (is_default_billing === true) {
      await supabase
      ...
      if (person?.default_billing_address_id === addressId) {
        await supabase
          .from('customer_persons')
    ...
    if (is_default_shipping === true) {
      ...
      if (person?.default_shipping_address_id === addressId) {
        await supabase
          .from('customer_persons')
    ...
    console.error('Error in person address PUT API:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
/**
 * DELETE /api/customers/persons/[id]/addresses/[addressId]
 * Soft delete an address for a person
 */
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('Error deleting person address:', error)
      return NextResponse.json(
    ...
    console.error('Error in person address DELETE API:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
