...
export default function CustomerBankAccountsCard({ 
  customerId, 
  initialBankAccounts, 
  currencies,
  onUpdate,
  ...
  // Update bank accounts when initialBankAccounts prop changes (after refresh)
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editingAccount
        ? `${basePath}/bank-accounts/${editingAccount.id}`
      ...
      // Fetch updated bank accounts from API
      try {
        const basePath = entityType === 'person' 
          ? `/api/customers/persons/${customerId}`
          : entityType === 'company'
          ? `/api/customers/companies/${customerId}`
          : `/api/customers/${customerId}`
        const refreshResponse = await fetch(`${basePath}/bank-accounts`)
        if (refreshResponse.ok) {
          const refreshData = await refreshResponse.json()
          setBankAccounts(refreshData.bank_accounts || [])
        }
      } catch (refreshError) {
        console.error('Error refreshing bank accounts:', refreshError)
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const basePath = entityType === 'person' 
        ...
        ? `/api/customers/companies/${customerId}`
        : `/api/customers/${customerId}`
      const response = await fetch(`${basePath}/bank-accounts/${deletingAccount.id}`, {
        method: 'DELETE'
      })
            ...
            <Table size="small">
              <TableHead>
                <TableRow sx={{ backgroundColor: '#fff8e1' }}>
                  ...
                  <TableCell sx={{ fontWeight: 600, py: 1, fontSize: '0.875rem' }}>Pénznem</TableCell>
              ...
              <TableBody>
                {bankAccounts.map((account) => (
                  <TableRow key={account.id} hover>
                    ...
                    <TableCell sx={{ py: 1 }}>
                      {account.currencies 
                        ? `${account.currencies.code} (${account.currencies.symbol})`
                        : '-'}
                    </TableCell>
                    <TableCell sx={{ py: 1 }}>
                      {account.is_default && (
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            ...
            <FormControl fullWidth>
              ...
              >
                <MenuItem value="">Nincs pénznem</MenuItem>
