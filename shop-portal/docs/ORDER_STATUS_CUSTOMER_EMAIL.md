...
- **Változók:** többek között `{{customer_firstname}}`, `{{order_number}}`, `{{status_label}}`, `{{payment_method_name}}`, `{{shipping_carrier}}`, `{{shipping_method_name}}`, `{{tracking_number}}` (ha nincs követés: **—**). `{{order_items_table}}` — szerver generálja a tételeket (kép, név, cikkszám, menny., sor bruttó; lábléc: rendelés bruttó végösszege). Csak a **levéltörzsben** használja (tárgyban figyelmen kívül hagyjuk).
- **Teljes HTML dokumentum** (`<!DOCTYPE>`, `<html>`, `<head>`, `<body style="…">`): mentéskor a `sanitizeEmailBodyHtml` kinyeri a `<body>` belsejét, a `body` inline stílusait egy `<div style="…">` burkolóra teszi, a `<head>`-beli `<style>` blokkokat a törzs elejére fűzi — így megmarad a háttérszín / betűtípus (korábban a parser eldobta a `<body>`-t).
