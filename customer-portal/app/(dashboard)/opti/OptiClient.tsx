...
// Props interface for OptiClient (Customer Portal Version)
interface OptiClientProps {
  ...
  customerData: any // Customer's own data from portal_customers
  initialQuoteData?: any // Portal quote data for editing (optional)
}
...
export default function OptiClient({ 
  ...
  const customers = [] // Not used in customer portal - customer data comes from customerData prop
  ...
  // Customer data state (Customer Portal: Pre-filled from logged-in customer)
  ...
  useEffect(() => {
    if (initialQuoteData && materials.length > 0 && edgeMaterials.length > 0) {
      console.log('[Customer Portal Opti] Loading portal quote for editing:', initialQuoteData.quote_number)
      
      setIsEditMode(true)
      setEditingQuoteId(initialQuoteData.id)
      setSavedQuoteNumber(initialQuoteData.quote_number) // Show quote number in save button
      
      // Customer data is already pre-filled from customerData prop
      // No need to update it (customer can't change their own data)
      
      // Populate panels from portal quote
      if (initialQuoteData.panels && initialQuoteData.panels.length > 0) {
        ...
        console.log(`[Customer Portal Opti] Loaded ${loadedPanels.length} panels from quote`)
      }
    }
  }, [initialQuoteData, materials, edgeMaterials])
  ...
  const quoteResult = useMemo<QuoteResult | null>(() => {
    ...
    // Cutting fee info constructed from initialCuttingFee prop
  ...
  const handleCustomerSelect = (customer: Customer | null) => {
    ...
    if (customer) {
      ...
      setCustomerFormData(newCustomerData)
    } else {
      setCustomerFormData({
        name: '',
        email: '',
  ...
  // Save Quote function (Customer Portal: saves to portal_quotes)
  const saveQuote = async () => {
    ...
    if (!customerFormData.name.trim()) {
      toast.error('Ügyfél neve hiányzik!')
      return
    }
    ...
    try {
      ...
      // Call API to save portal quote (Customer Portal: uses /api/portal-quotes)
      ...
      if (isEditMode) {
        ...
        // Redirect back to quote detail page after update
        setTimeout(() => {
          router.push(`/saved/${editingQuoteId}`)
        }, 1500)
      } else {
      ...
      console.error('[Customer Portal Opti] Error saving quote:', err)
        ...
        <Grid item xs={12} md={8}>
          <Card>
            <CardContent>
              ...
              <Grid container spacing={2}>
               {/* Customer Name (Customer Portal: Pre-filled and disabled) */}
               <Grid item xs={12} sm={8}>
                 <TextField
                   ...
                   required
                   helperText="Bejelentkezett ügyfél"
                 />
               </Grid>
               ...
               {/* Status - Customer Portal: Always shows logged-in customer */}
               <Grid item xs={12}>
                 <Box sx={{ display: 'flex', alignItems: 'center', height: '100%', pt: 1 }}>
                   <Typography variant="body2" color="text.secondary">
                     Az Ön adatai automatikusan kitöltve (nem szerkeszthető)
                   </Typography>
                 </Box>
               </Grid>
         ...
         >
                    ...
                    disabled={true}
                            ...
                            size="small"
                            label="Szálázási név"
                            value={customerFormData.billing_name}
                    ...
                    disabled={true}
                            ...
                            size="small"
                            label="Irányítószám"
                            value={customerFormData.billing_postal_code}
                    ...
                    disabled={true}
                            ...
                            size="small"
                            label="Utca"
                            value={customerFormData.billing_street}
                    ...
                    disabled={true}
                            ...
                            size="small"
                            label="Házszám"
                            value={customerFormData.billing_house_number}
                    ...
                    disabled={true}
                            ...
                            value={customerFormData.billing_tax_number}
                    ...
                    disabled={true}
                            ...
                            value={customerFormData.billing_company_reg_number}
        ...
        {addedPanels.length > 0 && (
          <Grid item xs={12}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mt: 3, mb: 2 }}>
              ...
              {/* Save Quote Button - Enabled in Customer Portal */}
              {optimizationResult && quoteResult && (
                <Button
                  ...
                  size="large"
                  onClick={saveQuote}
                  disabled={isSavingQuote || !customerFormData.name.trim()}
                  sx={{ 
                    minWidth: 200,
                    py: 1.5,
                    px: 4
