// Plain HTML template function for Portal Quote PDF (no React)
// Adapted from main app quote PDF template
...
interface PortalQuotePdfTemplateProps {
  ...
  vatRates?: any[] // Not used in template but kept for consistency
}
...
export default function generatePortalQuotePdfHtml({
  quote,
  tenantCompany,
  summary,
  discountAmount,
  ...
  // Build items array: materials + services (NO FEES for portal quotes)
  ...
  // NO FEES for portal quotes
  const itemsRows = materialRows + serviceRows.join('')
