...
export async function POST(
  ...
  console.log('[Customer-facing PDF] Starting...')
  ...
  try {
    ...
    const built = await buildOptiCustomerFacingHtml(quote_id, body, { preview: false })
    if (!built.ok) {
    ...
    try {
      ...
      console.error('[Customer-facing PDF] Puppeteer error:', puppeteerError)
    ...
    try {
      ...
      console.error('[Customer-facing PDF] PDF error:', pdfError)
      try {
        await browser?.close()
      } catch {
        /* ignore */
    ...
    console.error('[Customer-facing PDF] Unexpected error:', error)
    return NextResponse.json(
