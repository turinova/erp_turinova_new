...
export type NettfrontCustomerFacingPdfProps = {
  quote: {
    id: string
    quote_number: string
    created_at: string
...
export default function generateNettfrontCustomerFacingPdfHtml({
  quote,
  workshop,
  preparedBy,
  manualLines = [],
  ...
  const boardGross = Math.round(boardGrossCustomer)
  ...
  if (lineDisplay === 'collapsed') {
    boardRows = `
      <tr>
        <td>
          <div style="font-weight: 500;">Frontajtó / frontgyártás</div>
          <div style="font-size: 9px; color: #757575; margin-top: 0.25em;">Frontok és kapcsolódó díjak</div>
        </td>
        <td><span class="chip">Front</span></td>
        <td>—</td>
    ...
    const skuRows = (quote.sku_summary || [])
      .map(row => {
        ...
        const gross = (Number(row.gross) || 0) * factor
        ...
        const typeLabel =
          rawType === 'inomat' ? 'Inomat' : rawType.charAt(0).toUpperCase() + rawType.slice(1)
    ...
    const serviceRows = (quote.services || [])
      .map(svc => {
        ...
        const gross = (Number(svc.gross) || 0) * factor
  ...
  const lineRows = (quote.lines || [])
    .map(line => {
      const pant = line.panthely_holes_total > 0 ? `${line.panthely_holes_total} db` : '—'
      return `
      <tr>
...
<html lang="hu">
  ...
  <body>
    ...
    <table class="summary-table">
      <tbody>
        <tr class="summary-row">
          <td colspan="5" class="summary-row-bold">Bruttó (front):</td>
