-- Update create_pos_sale function to support per-item discounts
...
BEGIN
  ...
  -- Step 2: Calculate totals from items (with per-item discounts)
  ...
  -- Step 3: Calculate totals from fees (fees don't have per-item discounts)
  ...
  -- Step 6: Insert pos_order_items (products) with per-item discounts
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    v_product_type := COALESCE(v_item->>'product_type', 'accessory');
  ...
  -- Step 7: Insert pos_order_items (fees) - fees don't have per-item discounts
  FOR v_fee IN SELECT * FROM jsonb_array_elements(p_fees)
  LOOP
    -- Get VAT rate for this fee
    SELECT kulcs INTO v_vat_rate
