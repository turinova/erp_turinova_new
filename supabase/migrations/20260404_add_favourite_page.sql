-- Add Asztalosok (favourite customers) page to permission system
INSERT INTO public.pages (path, name, description, category, is_active)
VALUES (
  '/favourite',
  'Asztalosok',
  'Kedvenc ügyfelek gyorselérése',
  'Általános',
  true
)
...
-- Grant all users permission to access /favourite
DO $$
DECLARE
  user_record RECORD;
  page_id_var uuid;
BEGIN
  SELECT id INTO page_id_var FROM pages WHERE path = '/favourite';
