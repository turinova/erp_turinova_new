-- Fix create_pos_sale function to recalculate unit prices from base_price and multiplier
-- For materials: unit_price_net = base_price * multiplier (per m²)
-- For linear_materials: unit_price_net = base_price * multiplier (per m)
-- Calculate VAT from net price using VAT rate instead of subtracting prices
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_product_type = 'linear_material' AND v_item->>'linear_material_id' IS NOT NULL AND v_item->>'linear_material_id' != '' THEN
      ...
      END IF;
    ELSE
      -- For accessories, use prices from frontend
      v_unit_price_net := (v_item->>'unit_price_net')::numeric;
      v_unit_price_gross := (v_item->>'unit_price_gross')::numeric;
    END IF;
    
    -- Calculate totals: net, then VAT from net, then gross
    v_item_total_net := (v_item->>'quantity')::numeric * v_unit_price_net;
    v_item_total_vat := ROUND(v_item_total_net * v_vat_rate / 100, 2);
  ...
  LOOP
    ...
    -- Calculate totals: net, then VAT from net, then gross
    v_item_total_net := (v_fee->>'quantity')::numeric * v_unit_price_net;
    v_item_total_vat := ROUND(v_item_total_net * v_vat_rate / 100, 2);
    v_item_total_gross := v_item_total_net + v_item_total_vat;
