-- ============================================
-- Fix receive_shipment to check stock for ALL matching customer_order_items
-- Not just those linked via purchase_order_item_id
-- ============================================

-- Drop the old function
DROP FUNCTION IF EXISTS public.receive_shipment(uuid, uuid[]);

-- Create updated receive_shipment function that checks ALL matching customer_order_items
...
BEGIN
  ...
  -- FIXED: Check and update customer_order_items status to 'arrived' when stock becomes available
  -- Now checks ALL customer_order_items that match the product, not just those linked via purchase_order_item_id
  FOR v_item IN
    SELECT DISTINCT
      poi.product_type,
      poi.accessory_id,
  ...
  LOOP
    ...
    v_linear_material_id := v_item.linear_material_id;

    -- Check ALL customer_order_items that match this product (by FK), regardless of purchase_order_item_id
    FOR v_customer_item IN
      ...
      WHERE coi.deleted_at IS NULL
        ...
        AND coi.product_type = v_product_type
        AND (
          (v_product_type = 'accessory' AND coi.accessory_id = v_accessory_id AND v_accessory_id IS NOT NULL)
          OR (v_product_type = 'material' AND coi.material_id = v_material_id AND v_material_id IS NOT NULL)
          OR (v_product_type = 'linear_material' AND coi.linear_material_id = v_linear_material_id AND v_linear_material_id IS NOT NULL)
        )
    LOOP
      -- Check current stock across all warehouses for this product
      SELECT COALESCE(SUM(quantity_on_hand), 0) INTO v_available_stock
      FROM public.current_stock
      WHERE product_type = v_product_type
        AND (
          (v_product_type = 'accessory' AND accessory_id = v_accessory_id)
          OR (v_product_type = 'material' AND material_id = v_material_id)
          OR (v_product_type = 'linear_material' AND linear_material_id = v_linear_material_id)
        );
...
COMMENT ON FUNCTION receive_shipment(uuid, uuid[]) IS 
  'Receives a shipment and updates stock. FIXED: Now checks ALL customer_order_items that match the product (by FK), not just those linked via purchase_order_item_id. This allows items to be marked as arrived even if they were ordered through a different PO.';
