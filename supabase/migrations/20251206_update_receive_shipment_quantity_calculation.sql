-- ============================================
-- Update receive_shipment to calculate stock movement quantities based on product type
-- ============================================
-- 
-- For linear_materials: quantity_received * length / 1000 (convert mm to meters)
-- For materials: quantity_received * length_mm * width_mm / 1000000 (convert mm² to m²)
-- For accessories: quantity_received (no conversion)
--

-- Drop the old function
DROP FUNCTION IF EXISTS public.receive_shipment(uuid, uuid[]);

-- Create updated receive_shipment function with quantity calculation
...
BEGIN
  ...
  -- - linear_materials: quantity_received * length / 1000 (mm to meters)
  -- - materials: quantity_received * length_mm * width_mm / 1000000 (mm² to m²)
  -- - accessories: quantity_received (no conversion)
  FOR v_item IN
    SELECT 
      si.id,
      ...
      -- Calculate quantity based on product type
      CASE 
        WHEN poi.product_type = 'linear_material' AND lm.length IS NOT NULL THEN
          si.quantity_received * lm.length / 1000::numeric
        WHEN poi.product_type = 'material' AND m.length_mm IS NOT NULL AND m.width_mm IS NOT NULL THEN
          si.quantity_received * m.length_mm * m.width_mm / 1000000::numeric
        ELSE
          si.quantity_received  -- Accessories or fallback
      END AS calculated_quantity
    FROM public.shipment_items si
    INNER JOIN public.purchase_order_items poi ON poi.id = si.purchase_order_item_id
    LEFT JOIN public.materials m ON m.id = poi.material_id AND m.deleted_at IS NULL
    LEFT JOIN public.linear_materials lm ON lm.id = poi.linear_material_id AND lm.deleted_at IS NULL
    WHERE si.shipment_id = p_shipment_id
      AND si.deleted_at IS NULL
      AND poi.deleted_at IS NULL
      AND si.quantity_received > 0
  LOOP
    -- Insert stock movement with calculated quantity
    INSERT INTO public.stock_movements (
      ...
      v_item.calculated_quantity,  -- Use calculated quantity instead of quantity_received
      'in',
      'purchase_receipt',
      p_shipment_id,
      v_shipment.note,
  ...
  -- Check and update customer_order_items status to 'arrived' when stock becomes available
