...
/**
 * PATCH /api/worktop-orders/bulk-status
 * Update multiple worktop orders' status at once (with optional payment creation and SMS notifications)
 */
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (new_status === 'ready' && sms_order_ids.length > 0) {
      console.log(`[Worktop SMS] Fetching ${sms_order_ids.length} orders for SMS:`, sms_order_ids)
      ...
      if (ordersError) {
        console.error('[Worktop SMS] Error fetching orders:', ordersError)
      } else if (orders) {
        console.log(`[Worktop SMS] Fetched ${orders.length} orders from database`)
        ordersForSMS = orders.filter(order => 
          order.customers?.sms_notification === true && 
          order.customers?.mobile
        )
        console.log(`[Worktop SMS] Found ${ordersForSMS.length} orders eligible for SMS (${sms_order_ids.length} selected by user)`)
      }
    }
    ...
    if (create_payments) {
      ...
      if (ordersError) {
        console.error('Error fetching worktop orders for payment:', ordersError)
        return NextResponse.json(
      ...
      const { data: paymentSums, error: paymentsError } = await supabase
        ...
        .in('worktop_quote_id', order_ids)

      if (paymentsError) {
        console.error('Error fetching worktop payment sums:', paymentsError)
      }

      // Calculate total paid per order
      const paidByOrder = (paymentSums || []).reduce((acc, p) => {
        acc[p.worktop_quote_id] = (acc[p.worktop_quote_id] || 0) + p.amount
        return acc
      ...
      const { data: lastPayments, error: lastPaymentsError } = await supabase
        .from('worktop_quote_payments')
        .select('worktop_quote_id, payment_method')
        .in('worktop_quote_id', order_ids)
        .order('payment_date', { ascending: false })

      if (lastPaymentsError) {
        console.error('Error fetching last worktop payments:', lastPaymentsError)
      }

      const lastMethodByOrder = (lastPayments || []).reduce((acc, p) => {
        if (!acc[p.worktop_quote_id]) {
          acc[p.worktop_quote_id] = p.payment_method
        }
        return acc
      ...
      if (paymentsToCreate.length > 0) {
        ...
        if (createError) {
          console.error('Error creating worktop payments:', createError)
          return NextResponse.json(
    ...
    if (error) {
      console.error('Worktop bulk status update error:', error)
      return NextResponse.json(
    ...
    if (new_status === 'ready' && ordersForSMS.length > 0) {
      console.log(`[Worktop SMS] Sending ${ordersForSMS.length} SMS notifications...`)
      
      // Fetch company name from tenant_company table
      ...
      try {
        ...
        if (companyData?.name) {
          ...
          console.log(`[Worktop SMS] Using company name: ${companyName}`)
        }
      } catch (error) {
        console.error('[Worktop SMS] Error fetching company name, using default:', error)
      }
      ...
      for (const order of ordersForSMS) {
        ...
        if (result.success) {
          smsResults.sent++
          console.log(`[Worktop SMS] ✓ Sent to ${order.customers.name} (${order.customers.mobile})`)
          
          // Update ready_notification_sent_at timestamp (if column exists)
          ...
          if (timestampError) {
            console.error(`[Worktop SMS] Error updating ready_notification_sent_at for order ${order.id}:`, timestampError)
          }
        } else {
          smsResults.failed++
          smsResults.errors.push(`${order.customers.name}: ${result.error}`)
          console.error(`[Worktop SMS] ✗ Failed for ${order.customers.name}:`, result.error)
        }
      }
    }
