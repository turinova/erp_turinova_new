...
export async function PATCH(
  ...
  try {
    ...
    console.log('[COMMENT API] Updating worktop quote:', quote_id)
    ...
    if (updateError) {
      console.error('[COMMENT API] Error updating worktop quote comment:', updateError)
      return NextResponse.json(
    ...
    if (!data || data.length === 0) {
      ...
      return NextResponse.json(
        { error: 'Munkalap ajánlat nem található vagy már törölve' },
        { status: 404 }
      )
    }

    console.log('[COMMENT API] Successfully updated comment for worktop quote:', quote_id)
    return NextResponse.json({
      success: true,
      message: 'Megjegyzés sikeresen mentve',
    })
  } catch (error) {
    console.error('Error updating worktop quote comment:', error)
    return NextResponse.json(
      { error: 'Hiba történt a megjegyzés mentésekor' },
      { status: 500 }
    )
