...
export default function CreatePurchaseOrderModal({
  ...
  // Helper function to get display name from database or fallback to shop_order_items.product_name
  ...
  // Set default warehouse when modal opens or when warehouses load
  useEffect(() => {
  ...
  useEffect(() => {
    if (poOption === 'existing' && selectedExistingPoId) {
      ...
      // Reset to first warehouse when switching to new
      setWarehouseId(warehouses[0].id)
      setExpectedDate('')
    }
  }, [poOption, selectedExistingPoId, draftPOs, warehouses, warehouseId])

  // Initialize new item data with defaults only when modal opens
  useEffect(() => {
    if (open && pendingItems.length > 0 && newItemData.size === 0) {
      ...
      pendingItems.forEach(item => {
        initialData.set(item.id, {
          ...
          multiplier: item.multiplier || 1.38
        })
      })
  ...
  const fetchDraftPOs = async (partnerId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Filter by partner - need exact partner_id match
        const partnerDraftPOs = (data.purchase_orders || []).filter((po: any) => {
          // Match by partner_id (more reliable than name)
          return po.partner_id === partnerId
        })
        
        // Filter out POs that have source_type = 'customer_order' but might be in a shipment
        // For now, include all draft POs for this partner
  ...
  const handleCreatePO = async () => {
    ...
    setIsCreating(true)
    try {
      const res = await fetch('/api/shop-order-items/create-purchase-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          shop_order_item_ids: selectedItems.map(i => i.id),
  ...
  const handleCreateNew = (itemId: string, item: ShopOrderItem) => {
      ...
      <DialogContent>
        <Box sx={{ mb: 3, mt: 1 }}>
          <Alert severity="info" sx={{ mb: 2 }}>
            ...
            Beszállító: <strong>{selectedItems[0]?.partner_name}</strong>
          </Alert>

          {/* PO Selection - only show if draft POs exist */}
          {isLoadingDraftPOs ? (
