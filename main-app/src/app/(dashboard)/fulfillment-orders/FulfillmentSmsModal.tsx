...
interface FulfillmentSmsModalProps {
  open: boolean
  onClose: () => void
...
export default function FulfillmentSmsModal({
  open,
  onClose,
  onConfirm,
  orders,
  isProcessing = false
}: FulfillmentSmsModalProps) {
  ...
  const handleConfirm = async () => {
    // Allow confirmation even with 0 selected (will skip SMS)
    ...
    >
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          A következő ügyfelek SMS-t kapnak a rendelésükről. <strong>Minden termék megérkezett.</strong>
          <br />
        ...
        {orders.length === 0 && (
          <Box sx={{ py: 4, textAlign: 'center' }}>
            <Typography variant="body2" color="text.secondary">
              Nincs SMS-re jogosult rendelés
            </Typography>
          </Box>
        )}
      </DialogContent>
