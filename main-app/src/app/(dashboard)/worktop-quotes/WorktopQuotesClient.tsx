...
interface WorktopQuotesClientProps {
  initialQuotes: WorktopQuote[]
  totalCount: number
  totalPages: number
  currentPage: number
...
export default function WorktopQuotesClient({ 
  initialQuotes, 
  totalCount, 
  totalPages, 
  currentPage, 
  initialSearchTerm 
}: WorktopQuotesClientProps) {
  ...
  const [quotes, setQuotes] = useState<WorktopQuote[]>(initialQuotes)
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      router.push(`/worktop-quotes?${params.toString()}`)
    }, 500) // 500ms debounce
  ...
  useEffect(() => {
    if (!permissionsLoading && !hasAccess) {
      const timer = setTimeout(() => {
        toast.error('Nincs jogosultsága a Munkalap ajánlatok oldal megtekintéséhez!', {
          position: "top-right",
          autoClose: 3000,
    ...
    return (
      <Box sx={{ p: 3, display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' }}>
        <Typography variant="h6" color="error">
          Nincs jogosultsága a Munkalap ajánlatok oldal megtekintéséhez!
        </Typography>
      </Box>
    )
  }
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <TextField
        fullWidth
        placeholder="Keresés ügyfél nevében vagy árajánlat számban..."
        value={searchTerm}
            ...
            {quotes.length === 0 ? (
              ...
              quotes.map((quote) => (
                <TableRow 
                  ...
                  hover 
                  sx={{ cursor: 'pointer' }}
                  onClick={() => router.push(`/worktop-quotes/${quote.id}`)}
                >
      ...
      >
        <DialogTitle>Munkalap ajánlatok törlése</DialogTitle>
        <DialogContent>
          <DialogContentText>
