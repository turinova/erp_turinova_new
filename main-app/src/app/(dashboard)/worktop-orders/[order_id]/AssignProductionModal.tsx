...
export default function AssignProductionModal({
  ...
  useEffect(() => {
    if (open) {
      if (existingAssignment && existingAssignment.production_date) {
        ...
        // Pre-fill barcode if it exists (generated when order was created)
        setBarcode(existingAssignment?.barcode || '')
      }
      setError(null)
    }
  ...
  const isEditMode = existingAssignment && existingAssignment.production_date
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/worktop-quotes/${worktopQuoteId}/production`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          production_date: productionDate.toISOString().split('T')[0], // Format as YYYY-MM-DD
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Hiba a gyártásba adás során')
      }

      toast.success('Megrendelés sikeresen gyártásba adva!')
      onSuccess()
      onClose()
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || 'Hiba a gyártás törlése során')
      }
      ...
      <DialogContent>
        <Box sx={{ pt: 2 }}>
          ...
          <TextField
            ...
            required
            sx={{ mb: 2 }}
            helperText={existingAssignment?.barcode ? "Meglévő vonalkód (módosítható)" : "A vonalkód automatikusan generálva van a megrendelés létrehozásakor"}
            autoFocus
          />
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        {isEditMode && (
          <Button
            ...
            sx={{ mr: 'auto' }}
          >
            {isDeleting ? <CircularProgress size={16} /> : 'Törlés'}
          </Button>
        )}
        <Button 
          onClick={onClose}
